...
def clip_meter(img_original):
    ...
    # img_cliped = img_original[810 : 990, 1190 : 1680]
...
def normalize(img_meter):
    ...
    # cv2.imshow('img_red_c1', img_red_c1)
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()
    # cv2.imshow('img_green_c1', img_green_c1)
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()
    # cv2.imshow('img_meter', img_meter)
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()
    # kernel = make_sharp_kernel(3)
    # img_filtered = cv2.filter2D(img_thresh, -1, kernel).astype("uint8")
    # cv2.imshow('img_filtered', img_filtered)
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()

    # dst1 = cv2.medianBlur(img_blue_c1, ksize=7)
    # cv2.imshow('img_thresh', dst1)
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()
    ...
    # skeleton1   =   cv2.ximgproc.thinning(img_resized, thinningType=cv2.ximgproc.THINNING_ZHANGSUEN)
    # cv2.imshow('skeleton1', skeleton1)
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()

    # 細線化(スケルトン化) THINNING_GUOHALL 
    # skeleton2   =   cv2.ximgproc.thinning(img_resized, thinningType=cv2.ximgproc.THINNING_GUOHALL)
